...
Mounts at /war (separately deployable from chat-api). Routes:
  ...
  POST   /incident/<id>/event            — client-agent telemetry from teammate-client-hook
...
try:
    ...
    raise RuntimeError("FastAPI required: pip install 'claude-teammate[war]'") from exc

log = logging.getLogger(__name__)

app = FastAPI(title="teammate-war-api", version="3.0.0")
...
async def create_incident(req: CreateIncidentRequest):
    from teammate.war.alert_bridge import create_incident, IncidentSource
    try:
...
async def transition_incident(incident_id: str, req: TransitionRequest):
    from teammate.war.alert_bridge import transition, IncidentState
    try:
    ...
    if target == IncidentState.RESOLVED:
        from teammate.war.postmortem import draft_postmortem
        try:
...
async def slack_command(request: Request):
    ...
    if command == "/war":
        ...
        return _slack_response(f":rotating_light: *Incident {inc.id} created* — state: ACTIVE. War-room: <https://chat.teammate.placen.net/war/{inc.id}|open>")
