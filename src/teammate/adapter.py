...
or (b) accept that ``teammate ask`` doesn't see their local context.
...
::
    ...
    # ~/.teammate-adapter.toml  OR  <brain-root>/.teammate-adapter.toml
    [paths]
...
If both ``<brain-root>/.teammate-adapter.toml`` and ``~/.teammate-adapter.toml``
...
ADAPTER_FILENAME = ".teammate-adapter.toml"
...
def load_adapter(brain_root: Path) -> Adapter | None:
    ...
    Precedence: ``~/.teammate-adapter.toml`` (per-engineer) wins over
    ``<brain_root>/.teammate-adapter.toml`` (team-shipped fallback). When
...
_STARTER_ADAPTER = """\
# teammate adapter — per-engineer translation between your personal layout
# and the team brain's canonical layout. See docs/ADAPTER.md.
#
# This file lives at one of:
#   ~/.teammate-adapter.toml         (per-engineer, wins on conflict)
#   <brain-root>/.teammate-adapter.toml  (team-shipped fallback)
#
