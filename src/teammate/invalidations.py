"""Runtime integration — surface invalidation events at ``teammate ask`` time.

The companion to :mod:`teammate.impact`. ``impact`` writes events;
...
def _chunk_path(chunk: object) -> str:
    ...
    from :mod:`teammate.rag.ask`) or a plain dict with ``"path"``.
    """
...
def render_banner(
    ...
    *,
    ...
    """
    from teammate.impact import severity_at_least
    ...
    from teammate.impact import SEVERITY_RANK as RANK
