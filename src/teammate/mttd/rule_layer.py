...
class RuleLayer:
    ...
    def load_rules(self) -> list[WatchRule]:
        try:
            ...
            raise RuntimeError("PyYAML required: pip install 'claude-teammate[mttd]'")
    ...
    def sync_to_signoz(self, rules: list[WatchRule]) -> dict:
        ...
        with httpx.Client(headers=headers, timeout=30) as client:
            ...
            for name, rule in current.items():
                if name not in desired and rule.get("source") == "teammate":
                    rid = rule.get("id")
                    if rid:
...
def main() -> int:
    """CLI entry: teammate mttd sync-watchlist"""
