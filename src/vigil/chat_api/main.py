"""FastAPI app for vigil v1.

Run with:
    uvicorn vigil.chat_api.main:app --host 0.0.0.0 --port 8000
...
Env vars:
  ...
  TEAMMATE_NAMESPACE      default vigil-agent
...
def _build_prompt(query: str, hits: list[dict]) -> str:
    ...
    return (
        "You are vigil, an assistant grounded in the team's brain repo.\n"
...
async def reindex(request: Request) -> JSONResponse:
    ...
    jobs = batch.list_namespaced_job(NAMESPACE, label_selector="vigil-routine=reindex")
    ...
    cj = batch.read_namespaced_cron_job("vigil-reindex", NAMESPACE)
    ...
    job = k8s_client.V1Job(
        ...
        metadata=k8s_client.V1ObjectMeta(
            name=job_name,
            namespace=NAMESPACE,
            labels={"vigil-routine": "reindex", "requested-by": requested_by},
        ),
...
async def feed() -> dict:
    ...
    try:
        ...
        try:
            ...
            k8s_config.load_kube_config()
        batch = k8s_client.BatchV1Api()
        jobs = batch.list_namespaced_job(NAMESPACE, label_selector="vigil-routine")
        recent_jobs = sorted(
            jobs.items,
        ...
        return {
            "recent_jobs": [{
                ...
                "routine": j.metadata.labels.get("vigil-routine"),
