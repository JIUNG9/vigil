"""`vigil init` and `vigil scaffold` — set up the team brain on this machine.

Two distinct flows:

  - `vigil scaffold <dir>` — a TEAM LEAD creates a new team-brain repo
  ...
  - `vigil init` — an INDIVIDUAL ENGINEER sets up vigil in an
...
from vigil.providers import OllamaProvider, load_embedding_provider
...
def step_brain(brain_root: Path) -> dict[str, str]:
    ...
    if not brain.exists():
        return _fail(
            ...
            f"If you're the team lead setting this up, run `vigil scaffold <dir>` first."
        )
    stats = brain.stats()
    return _ok(
...
def step_gbrain(brain_root: Path, *, register: bool = False) -> dict[str, str]:
    ...
    if not register:
        return _ok(
            f"{status.notes} Re-run `vigil init --register-gbrain` "
...
def step_config(brain_root: Path) -> dict[str, str]:
    ...
    Existing ``.vigil/config.toml`` is never overwritten.
    """
...
def step_index(brain_root: Path) -> dict[str, str]:
    ...
    return _ok(
        ...
        f"Cache: .vigil-cache/vault.sqlite"
    )
...
def step_pre_push(brain_root: Path) -> dict[str, str]:
    ...
    if not git_dir.is_dir():
        return _skip(
            ...
            f"`vigil init --install-pre-push` again."
        )
