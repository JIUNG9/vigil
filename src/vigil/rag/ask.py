"""Retrieval + LLM orchestration: `vigil ask "<query>"`.
...
SYSTEM_PROMPT = f"""\
You are vigil, a battle buddy for SREs joining regulated teams. You are
...
def answer(
    ...
    *,
    ...
    if cache_dir is None:
        cache_dir = repo_root / ".vigil-cache"
    ...
    if not hits:
        ...
        yield (
            "No vault content matched. Run `vigil index` to populate the index, "
            "or `vigil init` to set it up.\n"
        )
        return
    ...
    if invalidations_cfg.enabled:
        ...
        if inv_root is None:
            ...
            inv_root = sibling if sibling.is_dir() else (
                Path.home() / ".vigil" / "brain-invalidations"
            )
        if inv_root.exists():
