...
On relevant events, creates Kubernetes Jobs for vigil routines.
...
Optional env vars:
  ...
  TEAMMATE_NAMESPACE        K8s namespace for Job creation (default: vigil-agent)
...
Fail-fast behavior:
  - Writes /tmp/vigil-heartbeat every 30s while socket is alive
  - Writes /tmp/vigil-ready once fully connected
...
def _create_k8s_job(routine: str, source: str = "slack-socket") -> str | None:
    ...
    namespace = os.environ.get("TEAMMATE_NAMESPACE", "vigil-agent")
    cronjob_name = f"vigil-{routine.replace('_', '-')}"
    ...
    try:
        jobs = batch.list_namespaced_job(
            namespace,
            label_selector=f"vigil-routine={routine}",
        )
    ...
    labels = {"vigil-routine": routine, "triggered-by": source}
...
def _heartbeat_thread(stop: threading.Event) -> None:
    heartbeat_path = Path("/tmp/vigil-heartbeat")
...
def run(poll_interval: int = 60, fail_on_disconnect: bool = True) -> int:
    ...
    try:
        ...
        log.error("slack-sdk not installed. Run: pip install 'claude-vigil[listen]'")
        return 1
    ...
    while True:
        try:
            ...
            sm_client.connect()
            Path("/tmp/vigil-ready").write_text("ok")
            ...
            if not initial_connect_announced:
                _notify_lifecycle(
                    f":white_check_mark: vigil listener connected — watching `{scope}`"
                )
        ...
        if reconnect_attempts >= MAX_RECONNECT_ATTEMPTS:
            ...
            _notify_lifecycle(
                f":x: vigil listener disconnected after "
