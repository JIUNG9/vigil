...
is team-defined via ``.vigil-naming.toml``. The shipped pattern is::
...
def load_naming_convention(path: Path | None) -> NamingConvention | None:
    """Load a ``.vigil-naming.toml`` from ``path``.
...
def find_naming_config(brain_root: Path) -> Path | None:
    """Search for a ``.vigil-naming.toml`` at the brain root."""
...
_TEMPLATE_BODIES: dict[str, str] = {
    "nexus-style": """\
# .vigil-naming.toml — nexus-style starter
...
""",
    "small-team": """\
# .vigil-naming.toml — small-team starter
...
""",
    "monorepo-only": """\
# .vigil-naming.toml — monorepo-only starter
...
""",
    "strict-iac": """\
# .vigil-naming.toml — strict-iac starter
# Pattern: {prefix}-{category}-{domain}-{service}[-{submodule}]-{type}
#
...
def write_starter(target: Path, template: str, *, force: bool = False) -> Path:
    """Write a starter ``.vigil-naming.toml`` to ``target``.
