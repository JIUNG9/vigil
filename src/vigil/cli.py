"""`vigil` CLI — entry point for the team-brain workflow.

Subcommands:

  vigil scaffold <dir>     — TEAM LEAD: create a new team-brain repo
                                 from the bundled template. One-time per org.
  vigil init               — ENGINEER: set up vigil inside an
                                 already-cloned team-brain repo.
  vigil adopt              — mid-project file migration. Walk an existing
                                 project, classify markdown, fill template
                                 gaps. ``--dry-run`` default; ``--apply`` opt-in.
  vigil validate           — read-only structural check: CLAUDE.md presence
                                 + size, link resolution, orphan files,
                                 non-canonical paths, frontmatter.
  vigil ask "<query>"      — query the brain locally (provider + RAG).
  vigil index [--rebuild]  — rebuild / refresh the local sqlite-vec index.
  vigil stats              — show what's in the brain (file counts by section).
  vigil config show        — print the effective provider config.
  vigil config init        — write a starter `.vigil/config.toml`.
  vigil doctor [--json]    — diagnostic: config source, reachability,
                                 model availability, index, proxy/CA env.
  vigil agent run <name>   — run a colleague-agent routine locally
                                 (mainly invoked by `/schedule` runners).
  vigil agent listen       — open a Slack Socket Mode WebSocket and trigger
                                 K8s Jobs for matching messages in real time.
                                 Requires `pip install claude-vigil[listen]`.
  vigil memory-import      — harvest team-relevant facts from
                                 ``~/.claude/`` memory into a review draft.
                                 Default for every entry is SKIP — opt in
                                 to import. Read-only on ``~/.claude/``.
  vigil memory-export      — departing-engineer flow; dump team-relevant
...
from vigil.adopt import adopt as run_adopt
...
from vigil.init import render_summary
from vigil.init import run as run_init
from vigil.init import scaffold as run_scaffold
from vigil.providers import (
    load_embedding_provider,
    load_llm_provider,
)
from vigil.rag.ask import answer
from vigil.rag.index import (
...
def main() -> None:
    """vigil — your team's brain in your team's git repo."""
...
def init(register_gbrain: bool, install_pre_push: bool) -> None:
    """Set up vigil in this already-cloned team-brain repo."""
    brain_root = Path.cwd()
    results = run_init(
        brain_root,
...
def ask(query: tuple[str, ...], rebuild: bool, top_k: int) -> None:
    ...
    brain_root = Path.cwd()
    cache_dir = brain_root / ".vigil-cache"
    ...
    if paths:
        try:
            ...
            click.echo("Hint: run `vigil index --rebuild`.", err=True)
            sys.exit(2)
...
@click.option("--output", "output_path", type=click.Path(path_type=Path),
              help="Write the index file to a custom path (default: .vigil-cache/vault.sqlite).")
...
def config_init(provider: str, force: bool) -> None:
    """Write a starter ``.vigil/config.toml`` for the given provider."""
    brain_root = Path.cwd()
    target = brain_root / ".vigil" / "config.toml"
    ...
    if provider not in {"ollama", "none"}:
        click.echo(
            ...
            f"vigil will fall back to keyword-only retrieval until v0.4.",
            err=True,
        )
...
def adopt(
    ...
    click.echo(f"vigil adopt — {mode} — wrote plan to {output_path}")
    click.echo(
...
@click.option("--include-naming", is_flag=True, default=False,
              help="Run the naming-convention check against directory names "
                   ...
                   ".vigil-naming.toml from the brain root.")
def validate(as_json: bool, max_claude_md_kb: int, include_naming: bool) -> None:
    ...
    # `[validate] include_naming` setting in .vigil/config.toml.
    ...
    if as_json:
        ...
        console = Console()
        console.print(f"[bold]vigil validate v{__version__}[/bold]\n")
...
def _check_index(brain_root: Path, cfg: TeammateConfig, embedder) -> dict[str, Any]:
    ...
    """
    db_path = brain_root / ".vigil-cache" / "vault.sqlite"
    if not db_path.exists():
        return _check_result(
            "index", _WARN,
            "no index yet — run `vigil index` to build it",
    ...
    stored_version = meta.get("vigil_version", "")
    ...
    if embedder is not None:
        ...
        if (stored_model, stored_dim) != (cfg_model, cfg_dim):
            return _check_result(
                ...
                f"current=({cfg_model}, {cfg_dim}d) — run `vigil index --rebuild`",
                provider=stored_provider, model=stored_model, dim=stored_dim,
                chunks=chunk_count, vigil_version=stored_version,
...
def _check_runtime() -> dict[str, Any]:
    ...
    return _check_result(
        "runtime", _PASS,
        f"python={py}  vigil={__version__}",
        python=py, vigil=__version__,
    )
...
def _render_report(checks: list[dict[str, Any]]) -> None:
    ...
    console = Console()
    console.print(f"[bold]vigil doctor v{__version__}[/bold]\n")
...
_NAMING_FILENAME = ".vigil-naming.toml"
...
def _resolve_naming_config(brain_root: Path) -> Path | None:
    """Locate ``.vigil-naming.toml`` for the active brain root."""
    from vigil.naming import find_naming_config
...
def naming_check(name: str) -> None:
    ...
    if cfg_path is None:
        click.echo(
            ...
            f"Run `vigil naming init` to write a starter.",
            err=True,
        )
        sys.exit(2)
...
def naming_list() -> None:
    ...
    from vigil.naming import load_naming_convention
...
def naming_init(template: str, force: bool) -> None:
    """Write a starter ``.vigil-naming.toml`` to the current directory."""
    from vigil.naming import list_templates, write_starter
    ...
    click.echo(
        ...
        f"run `vigil naming check <name>` to test."
    )
...
@click.option("--out-dir", "out_dir", type=click.Path(path_type=Path),
              default=None,
              help="Where the routine drops its report. Default: <brain>/.vigil-agent/.")
...
def agent_run(
    ...
    from vigil.agent.base import RoutineConfig
    ...
    if name not in list_routines():
        ...
        sys.exit(2)
    target_dir = out_dir if out_dir is not None else brain_root / ".vigil-agent"
    extra: dict[str, Any] = {}
...
def agent_listen(poll_interval: int, fail_on_disconnect: bool) -> None:
    ...
    Triggers K8s Jobs for matching vigil routines on Slack message, Jira
    issue, or Confluence page changes. Designed to run as a single-replica
    Kubernetes Deployment with a liveness probe on /tmp/vigil-heartbeat.
    ...
    """
    from vigil.socket_listener import run as socket_run
...
def memory_import(
    ...
    """
    from vigil.memory_import import harvest_user_memory, write_plan
...
def adapter_show() -> None:
    ...
    from vigil.adapter import ADAPTER_FILENAME, load_adapter
    ...
    if adapter_obj is None:
        ...
        click.echo("Run `vigil adapter init` to write a starter file.")
        return
...
@click.option("--scope", type=click.Choice(["home", "brain"]), default="home",
              show_default=True,
              help="Where to write the file. ``home`` = ~/.vigil-adapter.toml "
                   "(per-engineer). ``brain`` = <brain-root>/.vigil-adapter.toml "
...
def adapter_init(scope: str, force: bool) -> None:
    """Write a starter ``.vigil-adapter.toml`` based on detected layouts."""
    from vigil.adapter import ADAPTER_FILENAME, write_starter_adapter
...
def adapter_validate() -> None:
    ...
    from vigil.adapter import load_adapter, validate_adapter
...
def audit(since: str | None, query_grep: str | None, limit: int, as_json: bool) -> None:
    ...
    Audit lives at ``.vigil-cache/audit.jsonl`` and rotates weekly to
    ``audit-YYYY-WW.jsonl``. Both files are read by default.
    """
    import datetime as _dt

    from vigil.confidence import read_audit

    brain_root = Path(os.environ.get("TEAMMATE_BRAIN_ROOT") or Path.cwd())
    cache_dir = brain_root / ".vigil-cache"
    if not cache_dir.exists():
        click.echo("no audit log yet — run `vigil ask` first", err=True)
        sys.exit(0)
...
def _read_sync_section(brain_root: Path, name: str) -> dict[str, Any]:
    """Read ``[sync.<name>]`` from the repo's ``.vigil/config.toml``.
    ...
    cfg_path = brain_root / ".vigil" / "config.toml"
...
def _run_sync(routine: str, out_dir: Path | None, dry_run: bool) -> None:
    ...
    if dry_run:
        click.echo(
            ...
            f"`.vigil/config.toml` [sync.{routine}] "
...
def _parse_duration(value: str):
    ...
    Used by ``vigil impact list --since``. We accept only one unit per
...
def impact_preview(
    ...
    from vigil.impact import preview as run_preview
    ...
    console = Console()
    console.print(
        f"[bold]vigil impact preview[/bold]  resources={len(resources)}  "
...
def impact_emit(
    ...
    from vigil.impact import emit as run_emit
...
def impact_list(
    ...
    console = Console()
    console.print(
        f"[bold]vigil impact list[/bold]  window={since_str}  "
...
@click.option("--staging-dir", "staging_dir",
              ...
              help="Where the agent staged draft PRs. "
                   "Default: <brain>/.vigil-agent/draft-prs/.")
@click.option("--json", "as_json", is_flag=True,
...
def _render_brain_pulse(pulse) -> None:  # type: ignore[no-untyped-def]
    """Rich-rendered dashboard for ``vigil brain-pulse``."""
    from rich.console import Console
    ...
    console.print("Run `vigil ask \"...\"` to dig deeper.")
    console.print(rule)
...
def import_group() -> None:
    ...
    are incremental — the watermark is in ``.vigil-sync/state.json``.
...
def import_jira(dry_run: bool) -> None:
    ...
    """
    from vigil.importers.jira import JiraImporter
...
def import_confluence(dry_run: bool) -> None:
    ...
    """
    from vigil.importers.confluence import ConfluenceImporter
...
def import_github(dry_run: bool) -> None:
    ...
    """
    from vigil.importers.github import GitHubImporter
...
def import_slack(dry_run: bool) -> None:
    ...
    """
    from vigil.importers.slack import SlackImporter
