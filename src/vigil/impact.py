...
runbooks) lives in :mod:`vigil.validate`. They are different problems
...
Three commands wire onto this module:
  ...
  - ``vigil impact preview``  — pre-terraform hook. Greps the brain
  ...
  - ``vigil impact emit``     — post-terraform hook. Writes a
    structured JSON event to the brain-invalidations repo on disk.
  - ``vigil impact list``     — read recent events as a table.
...
def _resolve_invalidations_root(brain_root: Path,
    ...
    Order:
      ...
      3. ``~/.vigil/brain-invalidations`` (created on demand)
    """
    ...
    if sibling.is_dir():
        return sibling
    return Path.home() / ".vigil" / "brain-invalidations"
