"""Configuration loader for vigil.

Precedence (highest first):

  1. Environment variables (``TEAMMATE_LLM_*`` / ``TEAMMATE_EMBEDDING_*``)
  2. Per-repo config: ``<brain_root>/.vigil/config.toml``
  3. Per-user config: ``~/.vigil/config.toml``
...
def load_config(brain_root: Path) -> TeammateConfig:
    ...
    Reads (in order, last wins):
      1. defaults
      2. ~/.vigil/config.toml
      3. <brain_root>/.vigil/config.toml
      4. env vars
    """
    llm = _default_llm()
    ...
    user_path = Path.home() / ".vigil" / "config.toml"
    repo_path = brain_root / ".vigil" / "config.toml"
...
def write_starter_config(
    ...
    """Write a starter ``.vigil/config.toml`` under ``brain_root``.

    Returns the absolute path written. Caller decides whether to overwrite.
    """
    cfg_dir = brain_root / ".vigil"
    cfg_dir.mkdir(parents=True, exist_ok=True)
    cfg_path = cfg_dir / "config.toml"
    body = (
        "# vigil config — see docs/PROVIDERS.md\n"
        "# Precedence: env vars > this file > ~/.vigil/config.toml > defaults\n"
        "\n"
        ...
        "# Set SLACK_APP_TOKEN + SLACK_BOT_TOKEN env vars, then: vigil agent listen\n"
