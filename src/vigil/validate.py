"""`vigil validate` — read-only structural shape checks for a team brain.
...
# directories like `.git/` or `.vigil-cache/`.
...
def _read_validate_section(brain_root: Path) -> dict[str, Any]:
    """Read ``[validate]`` from ``.vigil/config.toml``. Returns ``{}`` on miss."""
...
def _check_naming_convention(brain_root: Path) -> CheckResult:
    ...
    Skipped silently when ``.vigil-naming.toml`` is absent — naming is
    ...
    if cfg_path is None:
        return CheckResult(
            ...
            status=PASS,
            summary="skipped — no .vigil-naming.toml",
...
def validate(
    ...
    *,
    ...
    = true`` in ``.vigil/config.toml``. The CLI flag wins over the TOML.
    """
